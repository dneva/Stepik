    ...
    public static class Spy implements MailService{
        ...
        public Sendable processMail(Sendable mail) {
            if (mail instanceof MailMessage){
                if(((MailMessage) mail).from.equals(AUSTIN_POWERS) || mail.getTo().equals(AUSTIN_POWERS)){
                    logger.log(Level.WARNING,"Detected target mail correspondence: from {0} to {1} \"{2}\"",new Object[]{((MailMessage) mail).getFrom(),((MailMessage) mail).getTo(),((MailMessage) mail).getMessage()});
                } else {
